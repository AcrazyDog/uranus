...
public class DesUtil {
	...
	public static void main(String args[]) throws UnsupportedEncodingException {
		...
		String result = DesUtil.encrypt(str.getBytes(), password);
		System.out.println("加密后：" + result);
		// 直接将如上内容解密
		try {
			String decryResult = DesUtil.decrypt(result, password);
	...
	public static String encrypt(byte[] datasource, String password) {
		try {
			...
			// 现在，获取数据并加密
			// 正式执行加密操作
			return parseByte2HexStr(cipher.doFinal(datasource)); // 按单部分操作加密或解密数据，或者结束一个多部分操作
		} catch (Throwable e) {
			e.printStackTrace();
		}
		return null;
